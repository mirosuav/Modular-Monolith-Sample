...
public readonly record struct Error(string Code, string Description, ErrorType ErrorType)
{
    ...
    {
        return NotFound("Error.NotFound", description);
    }
    ...
    {
        return Conflict("Error.Conflict", description);
    }

    public static Error Unauthorized(string description)
    {
        return new Error("Error.Unauthorized", description, ErrorType.Unauthorized);
    }
}
